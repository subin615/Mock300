...
function EditProduct(props) {
    ...
    }, [props.showEditProduct])//for model

    useEffect(() => {
        setproduct(props.selectedProduct)
    }, [props.selectedProduct]) //preious values inside model
    ...
    const editProduct = async () => {
        console.log(product);
        let url = `https://ty-shop.herokuapp.com/api/products/${product._id}`
     
        try {
            
                const response = await axios.put(url, product)
                console.log(response.data);
                afterAddUpdate(response)
